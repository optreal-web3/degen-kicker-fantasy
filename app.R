...
server <- function(input, output) {
  pbp <- reactive({
    tryCatch({
      ...
      load_pbp(2024) # Fallback to 2024 data if 2025 is unavailable
    })
  })
  ...
  compute_stats <- reactive({
    ...
    # Kicker stats
    fg_data <- tryCatch({
      pbp() %>% filter(week == week_num & play_type %in% c("field_goal", "extra_point") & !is.na(kick_distance) & kick_distance > 0)
    }, error = function(e) {
      data.frame()
    })
    if (nrow(fg_data) == 0) {
      ...
      fg_data <- fg_data %>%
        mutate(kick_points = case_when(
          ...
          play_type == "field_goal" & field_goal_result != "made" & kick_distance < 40 & !is.na(field_goal_result) ~ -3,
          play_type == "field_goal" & field_goal_result == "made" & kick_distance < 50 ~ 3,
          play_type == "field_goal" & field_goal_result != "made" & kick_distance < 50 & !is.na(field_goal_result) ~ -2,
          play_type == "field_goal" & field_goal_result == "made" & kick_distance < 60 ~ 4,
          play_type == "field_goal" & field_goal_result != "made" & kick_distance < 60 & !is.na(field_goal_result) ~ -1,
          play_type == "field_goal" & field_goal_result == "made" & kick_distance >= 60 ~ 5,
          play_type == "field_goal" & field_goal_result != "made" & kick_distance >= 60 & !is.na(field_goal_result) ~ 0,
          TRUE ~ 0
        ))
    ...
    # Punter stats
    punt_data <- tryCatch({
      pbp() %>% filter(week == week_num & play_type == "punt" & !is.na(kick_distance) & kick_distance > 0)
    }, error = function(e) {
      data.frame()
    })
    if (nrow(punt_data) == 0) {
